...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="body">
       ﻿<xsl:text>\documentclass[11pt, a4paper]{report}
  ...
  <xsl:template match="div[@type='textpart']">
      ...
      <xsl:text>}</xsl:text>
      \begin{verse}
      <xsl:apply-templates select="head|l|note|pb|fw"/>
      \end{verse}
  </xsl:template>
  ...
  <xsl:template match="l">
      <xsl:if test="text() != '\s*'"><xsl:value-of select="my:normalize(.)"/><xsl:text> \\ </xsl:text></xsl:if>
  </xsl:template>
